...
eval(fs.readFileSync(path.join(__dirname,"/lib/words.js")).toString());
...
class E3dcRscp {
	...
	initChannel() {
		...
		if( this.aesKey.write( this.config.rscp_password ) > this.config.rscp_password.length ) console.error("ERROR initializing AES-KEY!");
		...
		this.tcpConnection.connect( this.config.e3dc_port, this.config.e3dc_ip, () => {
			console.info("Connection to E3/DC is established");
			this.sendFrameFIFO();
		});
		...
		this.tcpConnection.on("data", (data) => {
			...
			if( this.inBuffer && this.inBuffer.length % 32 == 0 ) {
				...
				console.log(`inBuffer has length ${this.inBuffer.length} which is not a multiple of 256bit - waiting for next chunk...`);
			}
		});

		this.tcpConnection.on("end", () => {
			console.log("Disconnected from E3/DC");
			this.reconnectChannel();
		});

		this.tcpConnection.on("close", () => {
			console.log("E3/DC connection closed");
			this.reconnectChannel();
		});

		this.tcpConnection.on("timeout", () => {
			console.info("E3/DC connection timeout");
			this.reconnectChannel();
		});
		...
		console.debug(`Probing for PVI units - 0..${this.pviProbes-1}.`);
	...
	reconnectChannel() {
		if( !this.reconnectTimeout ) {
			console.info("Stop communication with E3/DC and pause a minute before retry ...");
	...
	addTagtoFrame( tag, sml = "", value = Object(0) ) {
		if( !rscpTagCode[tag] ) {
			console.log(`Unknown tag ${tag} with value ${value} - cannot add to frame.`);
			return;
		}
		...
		if( sml != "" && !Object.keys(this.pollingInterval).includes(tagCode) && tag.includes("_REQ_") && !tag.endsWith("_COUNT") ) {
			console.log(`${tag} has no assigned polling interval  - assuming 'M' - please check io-package.json`);
		...
		if( sml == "" || this.pollingInterval[tagCode] == sml ) {
			...
			switch( rscpType[typeCode] ) {
				case "None":
					break;
				case "Container":
					console.log(`Container-tag ${tag} passed to addTagToFrame - cannot add tag to frame.`);
					return;
				case "CString":
				...
				case "Timestamp": // NOTE: treating value as seconds - setting nanoseconds to zero
					...
					break;
				default:
					console.log(`addTagtoFrame does not know how to handle data type ${rscpType[typeCode]}`);
			}
			return 0;
		}
	}
	...
	startContainer( tag, sml = "" ) {
		if( !rscpTagCode[tag] ) {
			console.log(`Unknown container tag ${tag} - cannot start container.`);
			return 0;
		}
		const tagCode = rscpTagCode[tag];
		if( sml == "" || !Object.keys(this.pollingInterval).includes(tagCode) || this.pollingInterval[tagCode] == sml ) {
			...
			if( rscpType[typeCode] != "Container") {
				console.log(`Non-container tag ${tag} passed to startContainer - cannot start container.`);
				return 0;
			}
			const buf4 = Buffer.alloc(4);
			buf4.writeInt32LE( tagCode );
	...
	queueSetValue( globalId, value ) {
		console.info( `queueSetValue( ${globalId}, ${value} )`);
		...
		if( setTags && setTags.length == 2 ) {
			...
			console.log( `Don't know how to queue ${id}`);
		}
	}

	queueSetIdlePeriod( globalId ) {
		console.info( `queueSetIdlePeriod( ${globalId} )`);
		...
		if( el.length == 6 ) {
			...
			console.log(`queueSetIdlePeriod: invalid globalId ${globalId}`);
		}
	}

	queueGetHistoryData( globalId ) {
		console.info( `queueGetHistoryData( ${globalId} )`);
		const el = globalId.split(".");
		if( el.length == 5 ) { // e.g. "e3dc-rscp.0.DB.HISTORY_DATA_DAY.TIME_START"
					...
					this.getState( `${nameSpace}.${shortTag}.TIME_START`, (err, timeStart) => {
						this.getState( `${nameSpace}.${shortTag}.TIME_INTERVAL`, (err, interval) => {
							this.getState( `${nameSpace}.${shortTag}.TIME_SPAN`, (err, span) => {
								...
								this.endContainer(pos);
								this.pushFrame();
								console.debug(`TAG_DB_REQ_${shortTag} - START=${d.getTime()/1000} INTERVAL=${i} SPAN=${s}`);
							});
						});
					});
				}, this.config.send_tuple_delay*1000 );
			...
			console.log(`queueGetHistoryData: invalid globalId ${globalId}`);
		}
	}
	...
	sendFrame( f ) {
		if( f ) {
			if( rscpTag[f.readUInt32LE(18)] ) {
				console.debug( `Sending request ${rscpTag[f.readUInt32LE(18)].TagNameGlobal}` );
			} else {
				console.log( `sendFrame called with invalid content: first tag is ${f.readUInt32LE(18)}` );
			}
			console.log( `OUT: ${printRscpFrame(f)}` );
			// console.log( dumpRscpFrame(f) );
			...
			if( this.tcpConnection && this.tcpConnection.write( encryptedFrame ) ) {
				console.log( `Successfully written data to socket` );
			} else {
				console.error( `Failed writing data to socket` );
				this.initChannel();
			}
		}
	}
	...
	parseTlv( buffer, start, end ) {
		...
		while( start < end ) {
			...
			if( !rscpTag[tagCode] ) {
				console.log(`Unknown tag: tagCode=0x${tagCode.toString(16)}, len=${len}, typeCode=0x${typeCode.toString(16)}`);
				...
				switch( typeName  ) {
					...
					case "Error":
						...
						break;
					case "None":
						if( len > 0 ) console.log( `Received data type NONE with data length = ${len} - tagCode 0x${tagCode.toString(16)}` );
						break;
					default:
						console.log( `Unable to parse data: ${dumpRscpFrame( buffer.slice(start,start+7+len) )}` );
						value = null;
				}
	...
	processFrame( buffer ) {
		...
		if( magic != "E3DC" ) {
			console.log(`Received message with invalid MAGIC: >${magic}<`);
		}
		...
		switch( ctrl ) {
			...
			case "0011":
				...
				break;
			default:
				console.log(`Received message with invalid CTRL: >${ctrl}<`);
		}
		const dataLength = buffer.readUInt16LE(16);
		if( buffer.length < 18 + dataLength + (hasCrc ? 4 : 0) ) {
			console.log(`Received message with inconsistent length: ${buffer.length} vs ${18 + dataLength + (hasCrc ? 4 : 0)}`);
			console.debug( `IN: ${printRscpFrame(buffer)}` );
			console.log( dumpRscpFrame(buffer) );
		}
		if( hasCrc && (CRC32.buf(buffer.slice(0,18+dataLength)) != buffer.readInt32LE(18+dataLength))  ) {
			console.log(`Received message with invalid CRC-32: 0x${CRC32.buf(buffer.slice(0,18+dataLength)).toString(16)} vs 0x${buffer.readUInt32LE(18+dataLength).toString(16)} - dataLength = ${dataLength}`);
			console.log( dumpRscpFrame(buffer) );
		}
	...
	processTree( tree, path ) {
		console.log( `processTree: path = ${path}, tree = ${printTree(tree)}` );
		if( !tree ) return;
		let pathNew = path;
		...
		for( const i in tree ) {
			...
			if( typeName == "Error" ) {
				...
				if( ! ignoreIds.includes(shortId) ) {
					console.log( `Received data type ERROR: ${rscpError[token.content]} (${token.content}) - tag ${rscpTag[token.tag].TagNameGlobal} (0x${token.tag.toString(16)})` );
					continue;
				}
			}
			if( typeName == "Container" ) {
				...
				if ( multipleValueIds.includes(shortId) ) {
					...
					console.log( `storeValue( ${nameSpace}, ${pathNew + tagName + "."}, ${multipleValueIndex[shortId].toString().padStart(2,"0")}, ${rscpType[token.type]}, ${token.content}, ${tagName} )`);
				...
				if( mapReceivedIdToState[shortId] ) {
					...
					if( targetStateMatch && mapReceivedIdToState[shortId][targetStateMatch].targetState == "RETURN_CODE" && token.content < 0 ) {
						console.log(`SET failed: ${shortId} = ${token.content}`);
					}
				}
	...
	adjustTypeAndValue( shortId, typeName, value ) {
		...
		// if( typeName != newTypeName || value != newValue ) console.log(`adjustTypeAndValue(${shortId},${typeName},${value}}) returns [${newTypeName},${newValue}]`);
				...
				this.getState( `DB.${path}TIME_SPAN`, (err, spanObj) => {
					...
					tree.forEach( token => {
						if( rscpTag[token.tag].TagNameGlobal == "TAG_DB_SUM_CONTAINER" ) {
							...
							console.debug( `storeHistoryData: ignoring unexpected tag ${rscpTag[token.tag].TagNameGlobal}` );
							return; // next token
						}
	...
	deleteValueObjects( count, path ) {
		...
		//console.log(`deleteValueObjects: count=${count}, id=${id}`);
		this.getForeignObject( id, (err, obj) => {
			if(obj) {
				//console.log(`deleteValueObjects: found object with id=${id}`);
